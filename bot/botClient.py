...
class ServerData():
    ...
    def __init__(self, id, adminRole=0, prefix=defaultPrefix,
                 electionInstance=None, electionOpen=False):
        self.id = id
...
class VotingClient(discord.Client):
    ...
    async def run_command(self, messageText, messageContext):
        ...
        if(messageText.startswith(usedPrefix)):
            ...
            if(len(commandInput) > 1):
                ...
                exitCode = await self.commands.get(
                    command, self.noCommand)(messageContext, args)
            else:
                exitCode = await self.commands.get(
                    command, self.noCommand)(messageContext)
            return exitCode
        else:
            return 0
    ...
    async def on_message(self, message):
        if message.author != self.user:
            ...
            await self.run_command(message.content, message)

    async def createElection(self, ctx, paramList):
        p = self.servers[ctx.guild.id].prefix
        if await self.isAdmin(ctx):
            candidateString = " ".join(paramList)
            candidates = [i.strip() for i in candidateString.split(",")]
            if(self.servers[ctx.guild.id].electionInstance is None):
                self.servers[ctx.guild.id].electionInstance = \
                    voting.Voting(candidates)
                ...
                await ctx.channel.send("Election already in progress!\n "
                                       f"Use `{p}removeElection` to remove it,"
                                       f" or `{p}electionWinner` to calculate "
                                       "the previous election's winner!")
        ...
        if await self.isStarted(ctx):
            if(self.servers[ctx.guild.id].electionInstance is None):
                ...
                try:
                    replaced = False
                    if(ctx.author.id in
                       self.servers[ctx.guild.id].voteList.keys()):
                        (self.servers[ctx.guild.id].electionInstance
                            .removeVote(self.servers[ctx.guild.id]
                                        .voteList[ctx.author.id]))
                        replaced = True
                    voteID = (self.servers[ctx.guild.id]
                              .electionInstance.addVote(voteToMake))
    ...
    async def removeVote(self, ctx):
        if await self.isStarted(ctx):
            if(self.servers[ctx.guild.id].electionInstance is None):
                ...
                if(ctx.author.id in self.servers[ctx.guild.id]
                   .voteList.keys()):
                    (self.servers[ctx.guild.id].electionInstance.
                        removeVote(self.servers[ctx.guild.id]
                                   .voteList[ctx.author.id]))
                    ...
                    await ctx.channel.send("You have not yet voted"
                                           " in this election!")
    ...
    async def openElection(self, ctx):
        ...
        if await self.isAdmin(ctx):
            if(self.servers[ctx.guild.id].electionInstance is None):
                await ctx.channel.send("Election not yet created. "
                                       f"Run `{p}createElection [candidates]` "
    ...
    async def electionWinner(self, ctx):
        ...
        if await self.isAdmin(ctx):
            if(self.servers[ctx.guild.id].electionInstance is None):
                ...
                winner, endvotes = self.servers[ctx.guild.id].\
                    electionInstance.calculateResult()
                self.servers[ctx.guild.id].electionOpen = False
                self.servers[ctx.guild.id].electionInstance = None
                if winner is not None:
                    resultList = [f"{candidate} --> {votes} votes"
                                  for candidate, votes in endvotes.items()]
                    resultString = "\n".join(resultList)
                    await ctx.channel.send("Election winner is: \n"
                                           f"**{winner}**\n\n"
                                           "At the time majority was "
                                           "achieved, results were:"
                                           f"\n{resultString}")
                else:
                    await ctx.channel.send("No votes were provided. "
                                           "Removed election.")
    ...
    async def closeBot(self, ctx):
        ...
        if await self.isAdmin(ctx):
            try:
                ...
                await ctx.channel.send("Server data deleted. "
                                       f"Run `{p}startUp adminRoleId` "
                                       "to start the bot up again.")
                return 1
            except Exception:
                return -1
    ...
    async def githubLink(self, ctx):
        await ctx.channel.send("Follow any possible future development of the "
                               "bot on Github: <https://github.com/"
                               "pixeledbrain/DiscordRunoffVotingBot>")

    async def invite(self, ctx):
        await ctx.channel.send("Invite me to a server using: "
                               "<https://discord.com/oauth2/authorize?"
                               "client_id=830927490482569217&"
                               "permissions=347200&scope=bot>")

    async def commands(self, ctx):
        await ctx.channel.send("Look at the README for commands: "
                               "<https://github.com/pixeledbrain/"
                               "DiscordRunoffVotingBot> \n"
                               "Or just contact the creator for help: "
                               "PixeledBrain#0070 "
                               "(I'll make an actual commands command "
                               "someday, sorry)")
    ...
    async def isAdmin(self, ctx):
        if await self.isStarted(ctx):
            return (self.servers[ctx.guild.id].adminRole in
                    [i.id for i in ctx.author.roles] or
                    ctx.author.guild_permissions.administrator)
        elif ctx.author.guild_permissions.administrator:
            await ctx.channel.send("Bot not yet started up for this server. "
                                   "Run `!startUp adminRoleId` to start "
                                   "the bot up.")
            return False
